...
Create habits using exponential email reminders.
...
icanhabit relies extensively on environment settings which **will not work with Apache/mod_wsgi setups**. It has been deployed successfully with both Gunicorn/Nginx and even uWSGI/Nginx.

For configuration purposes, the following table maps the 'icanhabit' environment variables to their Django setting:
...
DJANGO_DEFAULT_FROM_EMAIL               DEFAULT_FROM_EMAIL          n/a                                            "icanhabit <noreply@icanhabit.com>"
DJANGO_SERVER_EMAIL                     SERVER_EMAIL                n/a                                            "icanhabit <noreply@icanhabit.com>" 
DJANGO_EMAIL_SUBJECT_PREFIX             EMAIL_SUBJECT_PREFIX        n/a                                            "[icanhabit] "
======================================= =========================== ============================================== ======================================================================
...
.. code-block:: bash
    ...
    cd icanhabit
    celery -A icanhabit.taskapp worker -l info
...
To run the tests, enter the icanhabit/tests directory and run the following commands::
...
..  code-block:: bash
    ...
    git remote add dokku dokku@yourservername.com:icanhabit
    git push dokku master
    ssh -t dokku@yourservername.com dokku redis:create icanhabit-redis
    ssh -t dokku@yourservername.com dokku redis:link icanhabit-redis icanhabit
    ssh -t dokku@yourservername.com dokku postgres:create icanhabit-postgres
    ssh -t dokku@yourservername.com dokku postgres:link icanhabit-postgres icanhabit
    ssh -t dokku@yourservername.com dokku config:set icanhabit DJANGO_SECRET_KEY=RANDOM_SECRET_KEY_HERE
    ssh -t dokku@yourservername.com dokku config:set icanhabit DJANGO_SETTINGS_MODULE='config.settings.production'
    ssh -t dokku@yourservername.com dokku config:set icanhabit DJANGO_AWS_ACCESS_KEY_ID=YOUR_AWS_ID_HERE
    ssh -t dokku@yourservername.com dokku config:set icanhabit DJANGO_AWS_SECRET_ACCESS_KEY=YOUR_AWS_SECRET_ACCESS_KEY_HERE
    ssh -t dokku@yourservername.com dokku config:set icanhabit DJANGO_AWS_STORAGE_BUCKET_NAME=YOUR_AWS_S3_BUCKET_NAME_HERE
    ssh -t dokku@yourservername.com dokku config:set icanhabit DJANGO_MAILGUN_API_KEY=YOUR_MAILGUN_API_KEY
    ssh -t dokku@yourservername.com dokku config:set icanhabit DJANGO_MAILGUN_SERVER_NAME=YOUR_MAILGUN_SERVER
    ssh -t dokku@yourservername.com dokku run icanhabit python manage.py migrate
    ssh -t dokku@yourservername.com dokku run icanhabit python manage.py createsuperuser
...
application stacks on one host, remove the port setting entirely and add `VIRTUAL_HOST=icanhabit.com` to your env file.
This pass all incoming requests on `nginx-proxy` to the nginx service your application is using.

.. _nginx-proxy: https://github.com/jwilder/nginx-proxy

Postgres is saving its database files to `/data/icanhabit/postgres` by default. Change that if you wan't
...
Place it in `/etc/supervisor/conf.d/icanhabit.conf` and run::

    supervisorctl reread
    supervisorctl start icanhabit
